...
int Generator::ConvertToPostfixForm(int i)
{
	vector <int> Stack;
	int n = 0;
	for (; ((Data->LexTable[i + n].type != LSeparator) && (Data->LexTable[i + n + 2].type != LStart) && (Data->LexTable[i + n + 1].type != LStart)); ++n);	//Встановлення коректності та довжини вхідного масиву
	int k = i + n;
	for (; i <= k; i++)
	{
		TypeOfLex in;
