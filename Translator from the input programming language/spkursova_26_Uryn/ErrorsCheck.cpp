...
int ErrorsCheck::IsExpression(int i, FILE* ef)
{
	...
	for (; (Data->LexTable[nom].type != LSeparator) && (Data->LexTable[nom].type != LStart) && (Data->LexTable[nom].type != LDo); nom++)
	{
		if (Data->LexTable[nom].type == LRBraket)
		{
			if (!(IsOperation(Data->LexTable[nom + 1].type)) && (Data->LexTable[nom + 1].type != LSeparator) && (Data->LexTable[nom + 1].type != LRBraket) && (Data->LexTable[nom + 1].type != LStart) && (Data->LexTable[nom + 1].type != LDo))
			{
...
int ErrorsCheck::ErrorChecking()
{
	...
	for (j = 0;; j++)
	{
		...
		if (Data->LexTable[j].type == LTrLBraket || Data->LexTable[j].type == LTrRBraket)        //Пошук невідомих слів(не ідентифікаторів)
		{
			Err_num++;
			fprintf(ef, "line %d: \tUnknown symbol!(%s)\n", Data->LexTable[j].line, Data->LexTable[j].name.c_str());
		}
		...
		if (Data->LexTable[j].type == LWhile)				//перевірка while
		{
			...
			{
				Err_num++;
				fprintf(ef, "line %d: \t'(' expected after '%s'!\n", Data->LexTable[j + 1].line, Data->LexName[LWhile].name.c_str());
			}
